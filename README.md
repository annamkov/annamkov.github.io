...
Github is used to store files in a repository. You will use it to store the files required to host your website. Why use Github? Github is a version control system, which means that it keeps track of changes made to files and maintains a record of all previous versions. The content in your repository will always be up-to-date and easy to manage because you will not have to keep manual track of files. Github makes for a seamless collaberative environment because changes to files are recoverable and their latests versions are easily accessible. 
